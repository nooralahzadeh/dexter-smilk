...
public class TitleRedirectId {
	...
	public TitleRedirectId(Article article) {
		...
		if (article.isDisambiguation()) {
			id = "-" + id;
                ...
                //<fnoorala>
                typeInWiki=article.getTypeName();
                //</fnoorala>
                
	}
        
        //<fnoorala>
        public String getTypeInWiki() {
		return typeInWiki;
	}

        public void setTypeInWiki(String typeInWiki) {
		this.typeInWiki= typeInWiki;
	}
        //</fnoorala>
	public String getId() {
		return id;
	...
	public static class Parser implements RecordParser<TitleRedirectId> {
		...
		public String encode(TitleRedirectId tri) {
                    if(tri.title.length()>0)
			return tri.title + "\t" + tri.redirect + "\t" + tri.id;
                    return null;
		}
                public String encode(TitleRedirectId tri, boolean t) {
			return tri.title + "\t" + tri.redirect + "\t" + tri.id+ "\t"+tri.typeInWiki  ;
		}
