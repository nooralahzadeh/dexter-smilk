...
public abstract class AbstractSpotter implements Spotter {
	...
	public SpotMatchList filter(DexterLocalParams params, SpotMatchList sml) {
		...
		for (SpotMatchFilter filter : filters) {
			...
			if (filter == null)
				continue;
                        logger.info("{} filter is applying to the SpotMatch set ",filter);
        ...
        @Override
         public SpotMatchList filter(DexterLocalParams params, SpotMatchList sml,Document doc){
		...
		for (SpotMatchFilter filter : filters) {
			if (filter == null)
				continue;
			sml = filter.filter(params, sml,doc);
		}
		return sml;
