...
public abstract class Relatedness implements Comparable<Relatedness> {
	...
	protected Relatedness(Entity x, Entity y) {
		this();
		set(x, y);

	}

	public void set(Entity x, Entity y) {
		this.x = x;
		this.y = y;
              
                 
		Couple k = new Couple(x.getId(), y.getId());
	...
	public int hashCode() {
		final int prime = 31;
		int result = 1;
		result = prime * result + x.getId();
		result = prime * result + y.getId();
		return result;
	}
	 ...
	 *            the y to set
	 */
	public void setY(Entity y) {
		this.y = y;
	}

	public int[] outX() {
		return in.getNeighbours(x.getId());
	}

	public int[] outY() {
		return in.getNeighbours(y.getId());
	}
	...
	public String getNames() {
		...
		String xStr = ih.getLabel(x.getId());
		String yStr = ih.getLabel(y.getId());
