...
public class ExtractSpotsCLI extends AbstractCommandLineInterface {
    ...
    public static void main(String[] args) throws InterruptedException {
        ...
        List<String> type = Arrays.asList("Article", "Redirect", "Disambiguation");
        for (Article a : reader) {
            //<fnoorala>
            //just special type will be considered as entity
            if (type.contains(a.getTypeName())) {
                //</fnoorala>
                progress.up();
                int target = 0;
                ...
                if (a.isRedirect()) {
                    ...
                    //take a first bold in the page as a spot
                    if (a.getHighlights().size() > 0) {
                        
                        String firstBold=a.getHighlights().get(0);
                        String boldString="";
                        if (spotManager.process(firstBold).size()>0)
                             boldString=new ArrayList<String>(spotManager.process(firstBold)).get(0);
                        String title="";
                        
                         if (spotManager.process(a.getTitle()).size()>0)
                            title=  new ArrayList<String>(spotManager.process(a.getTitle())).get(0);
                       
                         if (!boldString.equalsIgnoreCase(title) && !title.equalsIgnoreCase("") && !boldString.equalsIgnoreCase("")) {
                            for (String spot : spotManager.process(firstBold)) {
                                 cli.writeLineInOutput(spot + "\t" + source + "\t"
                                    + source);
                            }
                        }
