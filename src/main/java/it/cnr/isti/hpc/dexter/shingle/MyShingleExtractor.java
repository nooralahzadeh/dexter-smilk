 ...
 */
public class MyShingleExtractor implements Iterable<Shingle> {
    private static final Logger logger = LoggerFactory
			.getLogger(MyShingleExtractor.class);
    ...
    public MyShingleExtractor(Article a) {
        this();
        addText(a.getTitle());
    ...
    // this(doc.getMention());
    // }
    public MyShingleExtractor(String text) {
        this();
        addText(text);
    }
    ...
    //<fnoorala>
    public MyShingleExtractor(List<Token> tokens) {
        this();
        addText(tokens);
    }
    
    public MyShingleExtractor(Token token) {
        this();
        addText(token);
    }
     private void addText(List<Token> tokens) {
        ...
        if (tokens == null || tokens.isEmpty()) {
            ...
            cleanedSentences.addAll(listOfTokens);
        }

   private void addText(Token token) {
    ...
    private void addTextWithPos(String text) {
        if (text == null || text.isEmpty()) {
            return;
        }
       ...
       // ps.tag(text);
        List<ArrayList<PosToken>> tkpos = ps.tagWithOffSet(text);
     
        for(int i=0;i<tkpos.size();i++){
           
            ArrayList<PosToken> sentence_Tokens=tkpos.get(i);
        
          	    
            List<Token> cleanTokens = new LinkedList<Token>();
 
            for (PosToken t : sentence_Tokens) {
               ...
               if (cleanToken.isEmpty()) {
                 ...
                 Token tok=new Token(cleanToken,t.getStart(),t.getEnd(),t.getPos(),t.getStartSE(),t.getEndSE());
    ...
    public MyShingleExtractor(String text,boolean withPos) {
        this();
        addTextWithPos(text);
    }
