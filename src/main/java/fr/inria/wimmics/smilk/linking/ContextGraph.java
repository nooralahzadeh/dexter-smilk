...
public class ContextGraph {
    ...
    private static final Logger logger = LoggerFactory.getLogger(ContextGraph.class);
    ...
    Map finalWeights;
     ...
     Map<MyNode,Double> prior;
    
    public ContextGraph(SpotMatchList sml, int window, SimilarityFactory similarity, RelatednessFactory relatedness, Double alpha) {
        ...
        finalGraph = new UndirectedSparseGraph<>();
        finalWeights = new HashMap<MyLink, Double>();
        prior=new HashMap<MyNode,Double>();
        ...
        for (int i = 0; i < sml.size(); i++) {
            ...
            for (EntityMatch e : s.getEntities()) {
                ...
                if (labelsim > 0) {
                    ...
                    finalGraph.addEdge(edge, findMentionNode(mentionNodes, s), findCandidateNode(candidateNodes, e), EdgeType.UNDIRECTED);

                    // weighted.put(edge,e.getCommonness());   
                    finalWeights.put(edge, localsim);
                }
    ...
    private void makeGraph(SpotMatch b, SpotMatch a) {
              ...
              //   System.out.println(bem.getEntity().getName()+":" + rel);
                if (rel > 0) {
                    ...
                    if (!alreadyCreated(createdLinks, edge_e1_e2)) {
                        ...
                        finalGraph.addEdge(edge_e1_e2, findCandidateNode(candidateNodes, aem), findCandidateNode(candidateNodes, bem), EdgeType.UNDIRECTED);

                    }
                    rel = (rel > 0) ? rel : 0;
                    finalWeights.put(edge_e1_e2, rel);
                }
            }
    ...
    public EntityMatchList pageRankLike(int max_iteration, double epsilon, double lambda) {
        ...
        for (int i = 0; i < numVertices; i++) {
            if (vertices.get(i).mention != null) {
                 ...
                 */
                S.set(i, vertices.get(i).mention.getProbability());
            } else {
                candidateRows.add(i);
                S.set(i, vertices.get(i).candidate.getCommonness());
            }
        }
...
//            }
    ...
    public EntityMatchList PageRankJung(int max_iteration, double epsilon, double lambda) {
       
        EntityMatchList eml = new EntityMatchList();

        Transformer edge_weights = org.apache.commons.collections15.functors.MapTransformer.getInstance(finalWeights);

        //prior            
         for (MyNode pmid : finalGraph.getVertices()) {
             if(candidateNodes.contains(pmid)){
                Double commenness=pmid.candidate.getCommonness();
                commenness=(commenness.isInfinite() || commenness.isNaN())? 0: commenness;
                prior.put(pmid, commenness);
             }
             
             if(mentionNodes.contains(pmid)){
                 prior.put(pmid, (double)1);
             }
         }
        
        Transformer<MyNode, Double> vertex_prior = 
           new Transformer<MyNode, Double>()
           {            
        @Override
                public Double transform(MyNode v) 
                {                        
                    return prior.get(v);            
                }           
           };
        ...
        PageRankWithPriors<MyNode,Double> ranker=new PageRankWithPriors(finalGraph, edge_weights,vertex_prior, 0.15f);
        ranker.setTolerance(epsilon) ;
	ranker.setMaxIterations(max_iteration);
        ranker.evaluate();
         
        MultiValueMap  mention_candidateRanks = new MultiValueMap ();
        
    
    // Use priority queue to sort vertices by PageRank values.
    PriorityQueue<Ranking<MyNode>> q = new PriorityQueue<Ranking<MyNode>>();
    int i = 0;
    ...
    for (MyNode pmid : finalGraph.getVertices()) {
         ...
         if(candidateNodes.contains(pmid)){
            pmid.candidate.setScore(ranker.getVertexScore(pmid));
            mention_candidateRanks.put(pmid.origin, pmid.candidate);
         }
         
       q.add(new Ranking<MyNode>(i++, ranker.getVertexScore(pmid), pmid));
    }

    // Print PageRank values.
   //ystem.out.println("\nPageRank of nodes, in descending order:");
    Ranking<MyNode> r = null;
   
//    while ((r = q.poll()) != null) {
//         //  System.out.println(r.rankScore + "\t" + r.getRanked());
//    }
    
    Iterator<MyNode> it = mention_candidateRanks.keySet().iterator();
    
        while(it.hasNext()) {
           
            List<EntityMatch> list=(List<EntityMatch>) mention_candidateRanks.get(it.next());
            Collections.sort(list);
            
            for (EntityMatch c : list) {
                  eml.add(c);
      ...
      //  DegreeScorer degree = new DegreeScorer(finalGraph);
     
//        for (MyNode v : finalGraph.getVertices()) {
//            System.out.println(v.name + "--" + degree.getVertexScore(v));
//        }
//
//    HITSWithPriors<MyNode, Double > hits=new HITSWithPriors(finalGraph, edge_weights,vertex_prior, 0.15f);
//       
//    for (MyNode v : finalGraph.getVertices()) {
//            System.out.println(v.name + "--" + hits.getVertexScore(v).authority);
//        }
//        
